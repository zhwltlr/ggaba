...
- [ ] 환경 변수 Feature Flag 추가: `NEXT_PUBLIC_ENABLE_AI_DIAGNOSIS=false`
- [ ] 기존 `/diagnosis` 라우트를 `(hidden)/diagnosis` 라우트 그룹으로 이동
- [ ] BottomNav에서 "진단" 탭 조건부 렌더링 (Feature Flag 기반)
- [ ] 홈 페이지 CTA에서 진단 관련 UI 조건부 숨김
- [ ] `/vault` 페이지 Feature Flag 적용 (진단 결과 금고)

### 0-2. DB 스키마 확장 (Drizzle)

- [ ] `users` 테이블 확장:
  - [ ] `role` 컬럼 추가 — `'consumer' | 'contractor'` (기본값: `'consumer'`)
  - [ ] `business_profile_id` 컬럼 추가 — `business_profiles` FK (nullable)
- [ ] `business_profiles` 테이블 생성:
  - [ ] 사업자 정보: 상호명, 사업자번호, 대표자명, 연락처
  - [ ] 인증 상태: `verified` boolean
  - [ ] 전문 분야, 시공 가능 지역
- [ ] `auctions` 테이블 생성:
  - [ ] `user_id` FK (소비자)
  - [ ] 요청 정보: region, size_pyeong, budget_min, budget_max, schedule, description
  - [ ] 상태: `status` — `'open' | 'bidding' | 'selected' | 'in_progress' | 'completed' | 'cancelled'`
  - [ ] 마감 시간: `deadline_at`
  - [ ] 주소/연락처: 시공사에게 초기 비공개 (선택 후 공개)
- [ ] `bids` 테이블 생성:
  - [ ] `auction_id` FK, `contractor_id` FK
  - [ ] `total_price`, `message` (시공사 코멘트)
  - [ ] `status` — `'submitted' | 'selected' | 'rejected'`
  - [ ] Unique constraint: (auction_id, contractor_id) — 중복 입찰 방지
- [ ] `bid_items` 테이블 생성 — **기존 estimate_items와 동일한 4-tier 계층 구조**:
  - [ ] `bid_id` FK
  - [ ] `category` (대분류) → `detail` (세부항목) → `unit` (단위) → `unit_price` (단가)
  - [ ] `quantity`, `total_price`, `sort_order`
  - [ ] AI 진단 기능 재활용 시 데이터 호환성 보장
- [ ] `portfolios` 테이블 생성:
  - [ ] `contractor_id` FK
  - [ ] 시공 사례: title, description, region, size_pyeong, duration_days, total_cost
  - [ ] Before/After 이미지 URL 배열
- [ ] `reviews` 테이블 확장 (기존 테이블 업데이트):
  - [ ] `auction_id` FK 추가
  - [ ] `contractor_id` FK
  - [ ] rating (1-5), content, images
- [ ] `chat_rooms` 테이블 생성:
  - [ ] `auction_id` FK
  - [ ] `consumer_id` FK, `contractor_id` FK
...
- [ ] `messages` 테이블 생성:
  ...
  - [ ] `file_url` (nullable)
  - [ ] `read_at` (nullable)
- [ ] Drizzle relations 정의 (모든 신규 테이블 간 관계)
- [ ] 마이그레이션 생성 및 Supabase 반영
...
- [ ] `useUserStore` 확장: `userMode` 상태 추가 (`'CONSUMER' | 'CONTRACTOR'`)
- [ ] 모드 전환 함수: `toggleMode()` — 소비자 ↔ 시공사
- [ ] 시공사 모드 진입 조건: `business_profile_id` 존재 여부 확인
- [ ] 모드 상태 persist (localStorage)

### 1-2. Dynamic Layout — 모드별 탭바

- [ ] `BottomNav` 리팩터링: `userMode`에 따라 탭 구성 변경
  - [ ] **소비자 모드 탭:** 홈 / 경매 / 커뮤니티 / 마이페이지
  - [ ] **시공사 모드 탭:** 입찰목록 / 포트폴리오 / 커뮤니티 / 마이페이지
- [ ] 모드 전환 토글 UI (마이페이지 또는 상단 헤더)

### 1-3. Auth & Onboarding 업데이트

- [ ] 회원가입 후 온보딩 플로우:
  - [ ] 소비자: 최소 정보 수집 (닉네임)
  - [ ] 시공사: "사업자 인증" 단계 추가 (사업자번호, 상호명, 전문분야)
- [ ] 보호 라우트 업데이트:
  - [ ] 시공사 전용 라우트 (`/bids`, `/portfolio/edit`) — role 체크
  - [ ] 소비자 전용 라우트 (`/auction/new`) — role 체크
- [ ] middleware.ts 업데이트: 모드별 접근 제어
...
- [ ] `/` 홈 페이지 리디자인 (소비자 모드):
  - [ ] "시공 요청하기" CTA 버튼
  - [ ] 실시간 경매 현황 위젯 (내 경매 입찰 수, 상태)
  - [ ] 인기 시공사 / 최근 리뷰 미리보기

### 2-2. 경매 요청 폼 (Multi-step Wizard)

- [ ] `/auction/new` 멀티스텝 위저드:
  - [ ] Step 1 — 지역 선택 (region)
  - [ ] Step 2 — 평수 입력 (size_pyeong)
  - [ ] Step 3 — 예산 범위 (budget_min ~ budget_max)
  - [ ] Step 4 — 시공 일정 (schedule)
  - [ ] Step 5 — 상세 설명 + 사진 업로드
- [ ] **개인정보 보호:** 주소, 연락처는 시공사에게 초기 비공개
- [ ] Server Action: `createAuction(data)` — auctions INSERT
- [ ] 생성 후 `/auction/[id]` 상태 페이지로 이동

### 2-3. 경매 상태 페이지

- [ ] `/auction/[id]` 페이지:
  - [ ] 실시간 입찰자 수 표시 (Supabase Realtime 또는 polling)
  - [ ] 마감까지 남은 시간 타이머
  - [ ] 상태 뱃지: open → bidding → selected → in_progress → completed
  - [ ] 입찰 마감 전: "아직 입찰 진행 중" 안내

### 2-4. 입찰 비교 테이블

- [ ] `/auction/[id]/compare` 페이지:
  - [ ] 수신된 입찰 비교 테이블 렌더링
  - [ ] **컬러 코딩 로직:**
    - [ ] 초록 (Green): 해당 항목 최저가 구간
    - [ ] 회색 (Gray): 중간 구간
    - [ ] 빨강 (Red): 해당 항목 최고가 구간
  - [ ] **단가 기반 표시:** `m²당 단가` 또는 `개당 단가` 기준 비교 (총액이 아닌 unit price)
  - [ ] 시공사 포트폴리오 미리보기 링크
  - [ ] "이 시공사 선택" 버튼 → 채팅방 자동 생성
- [ ] Server Action: `getBidsForAuction(auctionId)` — 입찰 목록 조회
- [ ] Server Action: `selectBid(bidId)` — 입찰 선택 처리

### 2-5. 내 경매 목록

- [ ] `/auction` 페이지:
  - [ ] 내 경매 요청 목록 (상태별 필터)
  - [ ] 카드: 지역, 평수, 예산, 입찰 수, 상태 뱃지
  - [ ] 카드 클릭 → `/auction/[id]`
