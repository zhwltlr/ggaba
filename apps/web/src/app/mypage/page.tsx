...
export default function MyPage() {
  ...
  useEffect(() => {
    Promise.all([getProfile(), getMyPosts(), getMyComments()]).then(
      ([profileRes, postsRes, commentsRes]) => {
        setProfile(profileRes.profile as Profile | null);
        setMyPosts(postsRes.posts as Record<string, unknown>[]);
        setMyComments(commentsRes.comments as Record<string, unknown>[]);
        setLoading(false);
      }
    );
  }, []);
